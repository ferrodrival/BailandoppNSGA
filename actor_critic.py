...
import numpy as np
import models
from models.sep_vqvae import SepVQVAE
import datetime
    ...
    def eval(self):
       with torch.no_grad():
            ...
            for i_eval, batch_eval in enumerate(tqdm(self.test_loader, desc='Generating Dance Poses')):
                ...
                from nsga3 import main
                population = []
                for _ in range(50):  # Tamaño de población
                    zs = gpt.module.sample(x, cond=music_seq)
                    # Concatena up y down en un solo array numpy
                    z_combined = np.concatenate([
                        zs[0][0].cpu().numpy().flatten(), 
                        zs[1][0].cpu().numpy().flatten()
                    ])
                    population.append(z_combined)
                
                # Optimizar con NSGA-3
                zs = main(np.array(population))
